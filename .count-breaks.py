...
class Stanza:
    ...
    def generate_lines(self, breaks):
        ...
        self.listoflines = [ Line(pair[0], pair[1]) for pair in
...
def match_brackets(s,tag):
    ...
    return  [ find_tokens(x,tag) for x in strings ]
...
for file in os.listdir(directory):
    if file.endswith(".txt") :
        ...
        sys.stdout.write("\rCounting breaks..." + \
                         str(int(round((f*100)/float(number_of_files)))) + "%")
        ...
        for (x,y) in  zip(thispoem.stanzas , stanza_breaks):
            x.endswith(y)
